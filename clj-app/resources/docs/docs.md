...
The `home-routes` handler in the `clj-app.routes.home` namespace
...
The `home-page` function will in turn call the `clj-app.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `clj-app.handler` namespace:
...
Request middleware functions are located under the `clj-app.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `clj-app.dev-middleware` namespace found in
