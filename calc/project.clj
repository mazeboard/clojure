(defproject calc "0.1.0-SNAPSHOT"
  ...
  :ring {:handler calc.handler/app
         :uberwar-name "calc.war"}

  :min-lein-version "2.5.0"
  :uberjar-name "calc.jar"
  :main calc.server
  ...
  {:builds {:min
            ...
            {:source-paths ["src/cljs" "src/cljc" "env/dev/cljs"]
             :figwheel {:on-jsload "calc.core/mount-root"}
             :compiler
             {:main "calc.dev"
              :asset-path "/js/out"
              :output-to "target/cljsbuild/public/js/app.js"
  ...
  :profiles {:dev {:repl-options {:init-ns calc.repl}
